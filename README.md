...
A [jamovi](https://www.jamovi.org) module that extends the built-in **Descriptives**, **Scatter Plot**, and **Independent Samples T-Test** analyses:
...
- `dist/` — packaged `.jmo` build output per R version/OS/arch, generated by `tools/prepare-jmo.sh`/`tools/release.sh` (not tracked in git; attached to releases)
